...
top_aic_terms <- data.frame(statesList,statesList)
colnames(top_aic_terms) <- c("region","formula")
top_aic_terms$region <- as.character(top_aic_terms$region)
top_aic_terms$formula <- as.character(top_aic_terms$formula)
...
for (state in statesList){
  ...
  top_aic_terms$formula[[which(state == statesList)]] <- mod_formula[which.min(aic)]
...
write.table(modelAverage, file="estimates_topAIC_models_AR_corrected.csv", sep=',', row.names = F)#, col.names = statesList)
...
modelAverage <- data.frame(googlecsvData[1:417,1])
...
for (state in statesList){
  ...
  # Select only the columns of googleData that are meaningful (i.e. not all the same value of zero)
  ...
  #Use cochrane.orcutt to correct for autocorrleation and build 2^n-1 models
  ...
  # Initialize vectors to store model metrics
  ...
  # initialize list for top models
  topModels = c()
  ...
  # This is old code that was used before we realized that prediction of orcutt package doesnt add residuals
  ...
  # modelPrediction <- matrix(NaN, nrow = nrow(googleData), ncol = (2^(length(predictors))-1))
  # for (m in 1:(2^(length(predictors))-1)){
  #   orcutt_prediction <- data.frame(predict(all_models_results[[toString(m)]]))
  #   colnames(orcutt_prediction) <- c('prediction')
  #   for (index in 1:nrow(orcutt_prediction)){
  #     modelPrediction[strtoi(row.names(orcutt_prediction)[index]),m] <- orcutt_prediction$prediction[index]
  #   }
  # }
  ...
  for (m in 1:(2^(length(predictors))-1)){
    # if (m == topModels[1]){
    for (week in 1:nrow(googleDataModelTesting)){
      prediction <- 1*all_models_results[[toString(m)]]$coefficients[['(Intercept)']]
      for (variable in names(all_models_results[[toString(m)]]$coefficients)){
        origVariable <- variable
        if (substring(variable,1,1) == '`') 
          variable <- substring(variable, 2, nchar(variable)-1)
        if (variable != "(Intercept)"){
          prediction <- prediction + googleDataModelTesting[variable][week,1]*all_models_results[[toString(m)]]$coefficients[[origVariable]]
        }
      }
      if (is.na(tychoDataModelTesting$incidence[week]) == F){
        modelPrediction[week,m] <- prediction + all_models_results[[toString(m)]]$rho*residual
        residual = tychoDataModelTesting$incidence[week] - modelPrediction[week,m]
      }
      else{
        modelPrediction[week,m] <- NA
        residual = 0
      }
    }
    # }
  ...
  # figure out which models are lilely models, use all since this script is all models average
  for (i in 1:(2^(length(predictors))-1)) {
...
#Write the model prediction out to a csv file
write.table(modelAverage, file="estimates_all_models_averaged_AR_corrected.csv", sep=',', row.names = F)#, col.names = statesList)
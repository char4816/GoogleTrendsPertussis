...
# Generate a plot of all of the models across the entire timeseries timeframe for each state
...
pdf("C:/Users/chris/Google Drive/1Chris/1Research/2018 UROP/Scripts/plots_all_models_estimates_2004-2011.pdf",paper="a4r", width = 11, height = 8.5)
...
namesList = c('United States','ALASKA','ALABAMA','ARKANSAS','ARIZONA','CALIFORNIA','COLORADO','CONNECTICUT','DELAWARE','FLORIDA','GEORGIA','HAWAII','IOWA','IDAHO','ILLINOIS','INDIANA','KANSAS','KENTUCKY','LOUISIANA','MASSACHUSETTS','MARYLAND','MAINE','MICHIGAN','MINNESOTA','MISSOURI','MISSISSIPPI','MONTANA','NORTH CAROLINA','NORTH DAKOTA','NEBRASKA','NEW HAMPSHIRE','NEW JERSEY','NEW MEXICO','NEVADA','NEW YORK','OHIO','OKLAHOMA','OREGON','PENNSYLVANIA','RHODE ISLAND','SOUTH CAROLINA','SOUTH DAKOTA','TENNESSEE','TEXAS','UTAH','VIRGINIA','VERMONT','WASHINGTON','WISCONSIN','WEST VIRGINIA','WYOMING')
...
for (state in statesList){
  ...
  lines(plotData[,4]~plotData[,1], col=lineColors[4],lwd=1.0, tick = F)
  lines(plotData[,3]~plotData[,1], col=lineColors[3],lwd=1.0, tick = F)
  lines(plotData[,5]~plotData[,1], col=lineColors[2],lwd=1.0, tick = F)
  
  lines(plotData[,8]~plotData[,1], col=lineColors[7],lwd=1.0, tick = F)
  lines(plotData[,7]~plotData[,1], col=lineColors[6],lwd=1.0, tick = F)
  lines(plotData[,9]~plotData[,1], col=lineColors[5],lwd=1.0, tick = F)

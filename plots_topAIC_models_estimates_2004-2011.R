...
# Generate a plot of just the top AIC model for the entire timeseries timeframe for each state
...
pdf("C:/Users/chris/Google Drive/1Chris/1Research/2018 UROP/Scripts/12-9-2018-estimates_plots_ALLDATES_AR1.pdf",paper="a4r", width = 11, height = 8.5)
par(mar=c(5,6,4,1)+.1)
...
lineColors = c("gray30","deepskyblue")
...
for (state in statesList){
  ...
  plot(plotData$Project_Tycho[na.rm=T]~plotData[,1],type="l",col=lineColors[1],lwd=2.0,xlab="Date",ylab="Incidence per 100,000", main=namesList[which(statesList == state)], cex.main=2.0, cex.lab=2.0, cex.axis=1.75, ylim=c(0,max(plotData[,2:ncol(plotData)],na.rm=T)), tick = F)


  # lines(plotData[,5]~plotData[,1], col=lineColors[4],lwd=1.0, tick = F)
  # lines(plotData[,3]~plotData[,1], col=lineColors[2],lwd=1.0, tick = F)
  # lines(plotData[,4]~plotData[,1], col=lineColors[3],lwd=1.0, tick = F)
  lines(plotData[,9]~plotData[,1], col=lineColors[2],lwd=1.0, tick = F)
  # lines(plotData[,7]~plotData[,1], col=lineColors[6],lwd=1.0, tick = F)
  # lines(plotData[,8]~plotData[,1], col=lineColors[7],lwd=1.0, tick = F)
  ...
  # op <- par(cex = 0.75)
  legend(x = 'topright', lwd = 3, lty = rep(1, length(lineColors)), bty = "n", legend=c('PT','AR(1) AIC(i*)'), pch=rep(NA, length(lineColors)), col=lineColors, cex=1.5)
